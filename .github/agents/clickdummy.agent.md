...
handoffs:
  - label: Export Product
    agent: "Design: Export"
    prompt: "Clickdummy is assembled. Export the designs into the ai-coding workflow."
---
...
1. `design/product/product-overview.md` — Product name and description
2. `design/product/product-roadmap.md` — Section list for navigation
3. `design/src/shell/components/AppShell.tsx` — Shell component for wrapping
...
Also check for optional design tokens (`design/product/design-system/colors.json`, `design/product/design-system/typography.json`). If missing, show a brief note but continue — the screen designs should already have their own styling.

## Step 2: Inventory Sections

Read `design/product/product-roadmap.md` to get the complete list of sections.

For each section in the roadmap, check:

1. Does `design/product/sections/[section-id]/spec.md` exist?
2. Does `design/product/sections/[section-id]/data.json` exist?
3. Does `design/product/sections/[section-id]/types.ts` exist?
4. Does `design/src/sections/[section-id]/` contain at least one `.tsx` file? (This is the screen design preview wrapper)
...
For sections **with spec + data but no screen design** (has `spec.md` and `data.json` but no `.tsx` in `design/src/sections/[id]/`):
...
- `design/src/shell/components/AppShell.tsx` — Props interface (`AppShellProps`), especially `navigationItems`, `user`, `onNavigate`, `onLogout`, `children`
- `design/src/shell/components/MainNav.tsx` — The `NavItem` type: `{ label, href, icon: LucideIcon, isActive? }`
- `design/src/lib/section-loader.ts` — API functions: `getSectionScreenDesigns(sectionId)`, `loadScreenDesignComponent(sectionId, name)`
- `design/src/lib/product-loader.ts` — API function: `loadProductData()` returns `{ overview, roadmap }`
- `design/src/shell/ShellPreview.tsx` — Reference pattern for shell navigation with state-based active section

Also read one existing screen design preview wrapper (e.g., the first `.tsx` in `design/src/sections/[first-section]/`) to understand the pattern — these use `export default function` and import data internally.

### 4b: Create the Clickdummy Component

Create `design/src/clickdummy/ClickdummyApp.tsx`.
...
5. **Shell pattern** — Follow the exact same pattern as `design/src/shell/ShellPreview.tsx` for constructing `navigationItems` and handling `onNavigate`.

### 4c: Register the Route

Add the clickdummy route to `design/src/lib/router.tsx`.
...
3. Navigate to `http://localhost:5174/clickdummy/preview`
4. Verify:
...
"Your clickdummy is assembled at **http://localhost:5174/clickdummy/preview**
